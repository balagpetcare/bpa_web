...
export default function CareCardVerifyLandingPage() {
  ...
  function handleSearch(e: React.FormEvent) {
    ...
    setError('');
    router.push(`/verify/care-card/${encodeURIComponent(t)}`);
  }
      ...
      <section className="bg-gray-50 border-b border-gray-100 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="text-sm text-gray-400 mb-4 flex gap-2 items-center">
            ...
            <span>/</span>
            <span className="text-gray-600">Verify Care Partner Card</span>
          </nav>
          <h1 className="text-4xl font-bold text-(--bpa-navy)">Verify Care Partner Card</h1>
        ...
        <div className="max-w-lg mx-auto px-4">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 text-sm">
              Scan the QR code on the card or enter the verification token printed below the QR code.
            </p>
          </div>
          ...
          <form onSubmit={handleSearch} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">QR Token</label>
              <input
                type="text"
                value={token}
          ...
          <div className="mt-8 text-center text-sm text-gray-400">
            <Link href="/care-partner-card" className="text-(--bpa-green) hover:underline">
              Look up your card by contribution number →
            </Link>
          </div>
        </div>
      </div>
